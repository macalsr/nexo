...
- Routes:
  ...
  - `/verify-email` to consume an email-verification token from a link
...
- Centralized API access:
  ...
  - `src/api/authApi.ts` for `POST /auth/verify-email`
...
- Email verification is MVP-lightweight: users can enter `/app` before verification, but the app shell shows verification state and exposes a resend action.
...
- Automated email-verification coverage lives in `src/pages/VerifyEmailPage.test.tsx` and `src/pages/AppPage.test.tsx`.
